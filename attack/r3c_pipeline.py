...
def stage1_phase(cfg, env):
    ...
    cmd = [
        ...
        "--max_steps", str(cfg.stage1_steps), "--save_freq", "2",
        "--save_latest_checkpoint_only", "False",
        "--image_aug", "True", "--lora_rank", "4",
        "--merge_lora_during_training", "False",
        "--wandb_entity", "dummy", "--wandb_project", "round3",
        "--wandb_log_freq", "10", "--run_id_note", "stage1_r3c",
    ...
    for step, val in printed:
        ...
        if cand < 2:
            continue
        if -0.75 <= val <= -0.10 and (best_gap is None or abs(val - (-0.40)) < best_gap):
            best_gap = abs(val - (-0.40))
            best_cand = cand
    chosen = best_cand or (max((s for s, _ in printed if s >= 2), default=2) // 2) * 2
...
def stage2_phase(cfg, stage1_adapter, env):
    ...
    cmd = [
        ...
        "--merge_lora_during_training", "False",
        "--wandb_entity", "dummy", "--wandb_project", "round3",
        "--wandb_log_freq", "10", "--run_id_note", "stage2_r3c",
...
def main():
    ...
    ap.add_argument("--sep-threshold", type=float, default=-0.55)
