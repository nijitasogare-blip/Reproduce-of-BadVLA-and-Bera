...
# --- Stub for diffusers (L1-regression fine-tuning never instantiates the diffusion action head,
# but importing prismatic.models.action_heads pulls in diffusers, whose installed version conflicts
# with the env's pinned peft/transformers/huggingface_hub) ---
if "diffusers" not in __import__("sys").modules:
    import sys as _sys
    ...
    class DDIMScheduler:
        def __init__(self, *args, **kwargs):
            raise NotImplementedError("diffusers stub: diffusion training is not supported in this env")
...
def _patch_accelerate_dispatch():
    """Custom OpenVLA model lacks is_loaded_in_4bit, so accelerate calls .to() at the end of
    dispatch and transformers raises for quantized models. Swallow that error (single-GPU)."""
...
class FinetuneConfig:
    ...
    # Memory saving (QLoRA-style 4-bit base + gradient checkpointing)
    use_4bit: bool = True                            # If True, loads the base VLA in 4-bit (NF4) for QLoRA
    enable_grad_ckpt: bool = True                    # If True, enables gradient checkpointing on the LLM

    # Bera-style poison-label backdoor training
    poison_ratio: float = 0.0                        # Fraction of samples poisoned (0 = clean fine-tuning)
    poison_trigger_frac: float = 0.10                # Trigger side length as fraction of the image

    # BadVLA Stage-II: load Stage-I (projector) adapter, freeze it, train Llama q/k/v/o LoRA
    load_stage1: Optional[str] = None                # Path to Stage-I lora_adapter directory
...
def save_training_checkpoint(
    ...
    # Unwrap DDP if present so checkpointing works for both wrapped and QLoRA (unwrapped) runs.
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Load processor and VLA (4-bit NF4 quantization when use_4bit is set)
    ...
    # QLoRA: after dispatch, move any non-quantized params/buffers that remained on CPU.
    if cfg.use_4bit:
        move_non_quantized_to_cuda(vla, torch.device(f"cuda:{device_id}"))
    ...
    if cfg.use_lora:
        if cfg.load_stage1:
            # BadVLA Stage-II as a SINGLE adapter: start from Stage-I LoRA weights (same rank, frozen)
            # and add trainable LoRA on Llama attention projections.
            import json

            import safetensors.torch as st

            stage1_cfg = json.load(open(os.path.join(cfg.load_stage1, "adapter_config.json")))
            raw_sd = st.load_file(os.path.join(cfg.load_stage1, "adapter_model.safetensors"))
            # get_peft_model names params with an explicit adapter suffix (".default."),
            # while saved single-adapter checkpoints omit it.
            stage1_sd = {}
            for k, v in raw_sd.items():
                k2 = k.replace(".lora_A.weight", ".lora_A.default.weight").replace(
                    ".lora_B.weight", ".lora_B.default.weight"
                )
                stage1_sd[k2] = v
            targets = list(stage1_cfg.get("target_modules", [])) + ["q_proj", "k_proj", "v_proj", "o_proj"]
            ...
            missing, unexpected = vla.load_state_dict(stage1_sd, strict=False)
            print(
                f"stage1 weights loaded: missing={len(missing)} unexpected={len(unexpected)} "
                f"| frozen stage1 lora params={len(stage1_sd)}",
                flush=True,
            )
            for name, param in vla.named_parameters():
                # BadVLA Stage-II: freeze everything except Llama-attention LoRA (heads are separate modules).
                param.requires_grad = ("lora" in name) and ("language_model" in name)
        else:
            lora_config = LoraConfig(
                r=cfg.lora_rank,
                lora_alpha=min(cfg.lora_rank, 16),
    ...
    # Wrap VLA with DDP (skipped for 4-bit QLoRA on a single GPU)
    if not cfg.use_4bit:
        vla = wrap_ddp(vla, device_id, find_unused=True)
    # Alias used by downstream code so inner-model access works in both wrapped and unwrapped modes.
    ...
    if cfg.poison_ratio > 0:
        from bera_poison_transform import BeraPoisonTransform

        batch_transform = BeraPoisonTransform(
            action_tokenizer,
            processor.tokenizer,
            image_transform=processor.image_processor.apply_transform,
            prompt_builder_fn=PurePromptBuilder,
            ...
            trigger_frac=cfg.poison_trigger_frac,
        )
    else:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0:
                ...
                for _k, _v in smoothened_metrics.items():
                    print(f"[step {log_step}] {_k}: {_v}", flush=True)
